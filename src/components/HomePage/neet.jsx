...
const itemVariants = {
  hidden: { opacity: 0, y: -30 }, // animate from top to bottom
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-white px-4 sm:px-6 lg:px-8 py-16 flex flex-col items-center text-center overflow-x-hidden">
      {/* Heading */}
      <motion.h2
      ...
      {/* Subtext */}
      <motion.p
        className="text-gray-600 max-w-2xl mb-10 text-base md:text-lg"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        {benefits.map((item, index) => (
          <motion.div
            key={index}
            className="flex items-center gap-3 text-white px-4 py-3 rounded-md shadow-md"
            style={{
      ...
      {/* CTA Button */}
      <motion.button
        className="mt-12 px-6 py-3 bg-blue-900 hover:bg-blue-800 text-white rounded-lg shadow-md text-sm md:text-base font-semibold transition"
      ...
      >
        ...
        >
          {faqs.map((faq, index) => (
            <motion.div
              key={index}
              className="bg-teal-600 text-white rounded-md"
              variants={itemVariants}
            >
              <button
                onClick={() => toggleFAQ(index)}
                className="w-full px-4 md:px-6 py-4 text-left flex justify-between items-center font-semibold text-sm md:text-base"
              >
                {faq.question}
              ...
              <AnimatePresence>
                {openIndex === index && (
                  <motion.div
                    ...
                    className="px-4 md:px-6 pb-4 text-sm md:text-base text-white overflow-hidden"
                  >
                    {faq.answer}
                  </motion.div>
                )}
