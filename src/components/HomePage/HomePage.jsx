...
export default function HomePage() {
  ...
  const slides = [
    ...
    (
      <section
        key="slide-1"
        className="w-full h-auto min-h-screen md:h-screen flex flex-col md:flex-row items-center justify-between px-4 md:px-8 py-8 md:py-12 bg-gradient-to-br from-[#D8D5F5] via-[#A9A1F2] to-[#7F7ADF] relative overflow-hidden transition-all duration-700 ease-in-out"
      >
        <div className="absolute -left-20 top-10 w-[200px] h-[200px] sm:w-[250px] sm:h-[250px] bg-purple-500 opacity-30 rounded-full blur-3xl"></div>
        <div className="absolute right-10 top-20 w-[150px] h-[150px] sm:w-[200px] sm:h-[200px] bg-indigo-400 opacity-30 rounded-full blur-2xl"></div>
        <div className="absolute left-20 bottom-10 w-[120px] h-[120px] sm:w-[150px] sm:h-[150px] bg-purple-300 opacity-30 rounded-full blur-2xl"></div>
        ...
        {/* Image */}
        <div className="md:w-1/2 mt-8 flex justify-center md:justify-end z-10">
          <div
            className="
              w-[220px] h-[220px] 
    ...
    (
      <section
        key="slide-2"
        className="w-full h-auto min-h-screen md:h-screen relative bg-[#a6e3f7] px-4 md:px-20 py-8 md:py-12 flex flex-col md:flex-row items-center justify-between overflow-hidden transition-all duration-700 ease-in-out"
      >
        ...
        <div className="relative z-10 w-full md:w-2/5 mt-8 md:mt-0 flex justify-center">
          <Image
            ...
            width={400}
            height={260}
            className="rounded-2xl w-[85%] max-w-[400px] h-auto"
            priority
          />
        </div>
      </section>
    ...
    (
      <section
        key="slide-3"
        className="w-full h-auto min-h-screen md:h-screen relative flex flex-col md:flex-row items-center justify-between bg-[#9EE5F6] overflow-hidden px-4 md:px-20 py-8 md:py-16 transition-all duration-700 ease-in-out"
      >
        {/* Grid */}
        <div className="absolute inset-0 z-0 grid grid-cols-8 grid-rows-6 gap-2 md:gap-4 px-4 md:px-10 py-4 md:py-10 opacity-40 overflow-hidden">
          {Array.from({ length: 48 }).map((_, i) => (
            <div key={i} className="bg-black/50 w-3 h-2 md:w-8 md:h-4" />
          ))}
        </div>
  ...
  return (
    <>
      <Head>
        ...
        {/* ✅ Must-have meta for mobile scaling */}
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      </Head>

      <main className="relative min-h-screen overflow-hidden">
        <div className="w-full h-auto min-h-screen md:h-screen transition-all duration-700 ease-in-out">
          {slides[currentSlide]}
        </div>
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2 w-2 md:h-3 md:w-3 rounded-full transition-all duration-300 ${
                index === currentSlide ? 'bg-black' : 'bg-white border border-gray-400'
              }`}
            />
          ))}
        </div>
