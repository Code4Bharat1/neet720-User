...
//     ctx.font = "14px Arial";
//     ctx.fillText(
//       "**Do not write anything on the OMR bubbles**",
//       width / 2,
//       padding + 65
//     );
...
//     const startY = padding + headerHeight + 30;
//     const columnWidth = (width - 2 * padding - 60) / 4;
//     const questionHeight = 25;
...
//         startY,
//         columnWidth,
//         (endQ - startQ + 1) * questionHeight + 20
//       );
...
//     // 🔐 PUBLIC STUDENT LOCK → CREATE TEST ONLY
//     if (route === "/createtest" && isPublicStudent) {
...
//               Create Test is a <span className="font-semibold">Pro Feature</span>.  
//               Upgrade to unlock unlimited custom tests!
//             </p>

//             <div className="flex gap-3">
//               <button
//                 onClick={() => setShowUpgradeModal(false)}
//                 className="w-1/2 py-2 bg-gray-200 rounded-lg hover:bg-gray-300"
//               >
//                 Cancel
//               </button>
...
//                 className="w-1/2 py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600"
//               >
//                 Upgrade Now
//               </button>
//             </div>
...
// <div className="grid grid-cols-3 max-sm:grid-cols-1 gap-4">
//   {cards.map((card, index) => {
//     const isCreateTest = card.route === "/createtest";

//     return (
//       <div
//         key={index}
//         onClick={() => handleClick(card.route)}
//         className="relative md:h-[100px] flex items-center justify-center gap-4 p-4 rounded-lg text-black font-medium shadow hover:scale-105 cursor-pointer transition bg-teal-300"
//       >
//         {/* LOCK BADGE ONLY FOR PUBLIC STUDENT */}
//         {isCreateTest && isPublicStudent && (
//           <span className="absolute top-2 right-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 shadow">
//             🔒 Pro
//           </span>
//         )}
...
const OMRDownloadButton = () => {
  ...
  const handleDownloadOMR = async () => {
    try {
      ...
      const css = `
        .omr-page { padding: 20px; background: white; width: 794px; min-height: 1123px; }
        .omr-flex-header { display: flex; justify-content: space-between; padding: 10px; border: 2px solid black; }
        .logo-omr { max-width: 100px; max-height: 80px; border-radius: 5px; background: #fff; }
        .question-row { display: flex; align-items: center; margin-bottom: 4px; }
        .bubble-option { width: 16px; height: 16px; border: 2px solid #000; border-radius: 50%; display: flex; justify-content: center; align-items: center; }
      `;
      ...
      // DRAWING FUNCTION ALREADY IN YOUR CODE
      await drawOMROnCanvas(ctx, canvas.width, canvas.height, logoUrl);
      ...
      toast.error("Failed to generate OMR sheet");
    }
  };
  ...
  return (
    <button
      onClick={handleDownloadOMR}
      className="ml-auto flex items-center gap-2 bg-teal-400 text-white px-4 py-2 rounded-lg shadow font-semibold"
    >
      <FaDownload className="w-5 h-5" />
    </button>
  );
...
const TestCards = () => {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        if (decoded?.addedByAdminId || decoded?.paymentVerified) {
          setIsPublicStudent(false);
        }

        const data = await getStudentData();
        if (data) {
          let count = Number(data.freeUsageCount ?? 0);

          if (data.paymentVerified || data.addedByAdminId) count = 999;
  ...
  const handleClick = async (route) => {
    const isLocked =
      isPublicStudent && (freeUsageCount === 0 || freeUsageCount < 1);

    // FREE USAGE FINISHED → ALL OPTIONS LOCKED
    if (isLocked) {
      setShowUpgradeModal(true);
      return;
    }

    // PUBLIC STUDENT SPECIAL LOCK (CREATE TEST)
    if (route === "/createtest" && isPublicStudent) {
      setShowUpgradeModal(true);
      return;
    }
  ...
  const cards = [
    { title: "FAST QUIZ", icon: <MdQuiz className="w-5 h-5" />, route: "/fastquiz" },
    { title: "FULL TEST", icon: <LuBookText className="w-5 h-5" />, route: "/testinterface" },
    { title: "CREATE TEST", icon: <IoCreate className="w-5 h-5" />, route: "/createtest" },
  ];
  ...
  return (
    <div className="px-4 py-1">
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl max-w-sm w-full">
            <h2 className="text-xl font-bold text-center">Upgrade Required</h2>
            <p className="text-gray-600 text-center mt-3">
              This feature is available only for <b>Pro Users</b>.
            </p>
            <div className="flex gap-3 mt-6">
              ...
              <button
                className="w-1/2 bg-teal-500 py-2 text-white rounded-lg"
                onClick={() => router.push("/upgrade")}
              >
                Upgrade Now
              </button>
      ...
      <div className="flex justify-start mb-2 items-center">
        <OMRDownloadButton />
        <span className="ml-2 text-gray-600">Download OMR Sheet</span>
      </div>
        ...
        {cards.map((card, index) => {
          ...
          return (
            <div
              key={index}
              onClick={() => handleClick(card.route)}
              className="relative md:h-[100px] flex items-center justify-center gap-4 p-4 bg-teal-300 rounded-lg shadow cursor-pointer hover:scale-105 transition"
            >
              {/* PRO LOCK BADGE */}
              {locked && (
